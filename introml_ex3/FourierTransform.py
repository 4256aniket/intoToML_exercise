...
def calculateMagnitudeSpectrum(img):
    if img is None or img.size == 0 or len(img.shape) != 2:
        raise ValueError("Input image must be a non-empty 2D grayscale image.")
    
    ft = np.fft.fft2(img)
    magnitude_spectrum = 20 * np.log10(1 + np.abs(np.fft.fftshift(ft)))
